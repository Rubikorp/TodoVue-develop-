  ...
  <div class="box-find">
    <div class="find">
      <input
        ...
        type="text"
        :class="
          (getBlackScheme ? 'input-black-scheme ' : '') + 'find__input input' 
        "
        placeholder="Поиск задачи..."
        v-model="query"
      />
      <label @click="setSearchQueryBtn(query)"
        ><svg
          class="find__svg"
    ...
    <div class="box-select-div">
      <div class="box-select-div__btn">
        <button class="box-select-div__select" @click="clickOpenCloseSelect()">
          {{ current_select.value }}
          <svg
            class="box-select-div__svg"
            :class="check_click_select ? 'active-option-svg' : ''"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 8 5"
            fill="none"
          >
      ...
      <div
        :class="'box-select-options-div ' + (check_click_select ? 'active-option' : '')"
      >
        <ul class="box-select-list">
          <li
            class="box-select-list__item"
            v-for="item in select_option"
            :key="item.name"
            @click="clickOption(item.name)"
          >
            {{ item.value }}
          </li>
        </ul>
    ...
    <button class="btn-color" @click="toogleScheme">
...
export default {
  ...
  data() {
    return {
      ...
      select_option: [
        { name: "all", value: "Всё" },
        { name: "compl", value: "Сделано" },
        { name: "uncompl", value: "Не сделано" },
      ],
      current_select: { name: "all", value: "Всё" },
      check_click_select: false,
    };
  },
  methods: {
    ...mapMutations(['editFilter', 'setSearchQuery', 'toogleBlackScheme']),
    clickOption(name) {
    ...
    clickOpenCloseSelect() {
      let select = this.check_click_select;
      this.check_click_select = !select;
    },
    setSearchQueryBtn(queryText) {
      this.setSearchQuery(queryText)
      this.query = "";
    },
    toogleScheme() {
      const html = document.querySelector("html");
      if(html.classList.length === 0) {
        html.className = 'bg-black-scheme';
      } else {
        html.className = ''
      }
      this.toogleBlackScheme();
    }
  },
  computed: {
